...
export default function GoogleConsoleLoginPage() {
  ...
  useEffect(() => {
    if (!loading && user?.email && isAuthorizedEmail(user.email)) {
      router.replace("/google-console");
    }
  }, [loading, user, router]);

  useEffect(() => {
    // If user clicked the header icon, we land here with ?autologin=1
    // Start redirect sign-in once we know we are not already signed in.
  ...
  const authStatus = useMemo(() => {
    ...
    // keep UI clean: only show something when there's an error or an unauthorized signed-in state
    if (!loading && user?.email && !isAuthorizedEmail(user.email)) {
      return (
        <div className="mb-4 text-sm text-rose-700 bg-rose-50 border border-rose-200 p-3">
  ...
  return (
    <ConsoleLoginShell
      ...
      subtitle="Secure, Google-authenticated access to your investor console and live dashboards."
      redirectTo="/google-console"
